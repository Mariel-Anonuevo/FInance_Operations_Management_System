...
const MIGRATION_FLAG = 'arcms_migrated_v2';

if (typeof window !== 'undefined' && !localStorage.getItem(MIGRATION_FLAG)) {
  LEGACY_KEYS.forEach((key) => localStorage.removeItem(key));
  localStorage.removeItem('arcms_employees');
  localStorage.removeItem('arcms_user');
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [employees, setEmployees] = useState<Employee[]>(() => {
    ...
    return list.map((emp) => (emp.id === 'EMP-001' ? { ...emp, status: 'Active' as const } : emp));
  });
